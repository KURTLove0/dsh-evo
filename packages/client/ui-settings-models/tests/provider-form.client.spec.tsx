...
/** The llm-claude-cli driver section shape as the host serializes it. */
const ClaudeCliConfig = Schema.object({
  claude: Schema.object({
...
/** One CLI-family namespace view around the codex driver section. */
function claudeCliNamespace(
  codex: Record<string, unknown> | undefined,
  userCodex: Record<string, unknown> | undefined = codex,
): SettingsNamespaceView {
  return {
    ns: 'llm-claude-cli',
    schema: JSON.parse(JSON.stringify(ClaudeCliConfig.toJSON())) as unknown,
    value: { ...codex === undefined ? {} : { codex } },
    base: {},
    user: { ...userCodex === undefined ? {} : { codex: userCodex } },
    applies: 'live',
    secrets: [],
    revision: 3,
  }
...
function cliFace(options: {
  codexUser?: Record<string, unknown> | undefined
  /** Host catalog groups; the default carries the live codex route. */
  modelsGroups?: { id: string; name: string; models: { id: string; name: string }[] }[]
  modelsFail?: string
  mutate?: ReturnType<typeof vi.fn>
} = {}) {
  const codexUser = options.codexUser ?? {
    command: 'codex',
    provider: 'codex-cli',
    timeoutMs: 300_000,
    models: [{ id: 'gpt-5-codex', name: 'gpt-5-codex' }],
  }
  const namespace = claudeCliNamespace(codexUser)
  const mutate = options.mutate ?? vi.fn(() => Promise.resolve(ok(namespace)))
  const models = options.modelsFail !== undefined
    ? vi.fn(() => Promise.resolve(fail(options.modelsFail ?? 'down', 'internal')))
    : vi.fn(() => Promise.resolve(ok({
      groups: options.modelsGroups ?? [{
        id: 'codex-cli',
        name: 'Codex CLI',
        models: [{ id: 'gpt-5-codex', name: 'gpt-5-codex' }, { id: 'o4-mini', name: 'o4-mini' }],
      }],
      failures: [],
    })))
  const face = {
    llm: {
      providers: vi.fn(() => Promise.resolve(ok({ providers: [
        { provider: 'codex-cli', displayName: 'Codex CLI', settingsNs: 'llm-claude-cli', settingsPath: ['codex'], active: true },
      ] }))),
      models,
  ...
  return { face: face as never, mutate, models, namespace }
}

/** Mount one CLI-family driver card directly (no key field, connection fields up top). */
async function mountCliCard(options: Parameters<typeof cliFace>[0] = {}) {
  const { face, mutate, models, namespace } = cliFace(options)
  const view = namespace
  const { ProviderEditor } = await import('../src/client/ProviderEditor.tsx')
  const onClose = vi.fn()
  const renderResult = render(<ProviderEditor
    provider="codex-cli"
    ...
    onClose={onClose}
  />)
  return { ...renderResult, face, mutate, models, onClose, namespace: view }
}

describe('CLI-family provider editor (llm-claude-cli)', () => {
  it('renders connection fields and the runtime catalog instead of the hint', async () => {
    await mountCliCard()
    expect(screen.getByLabelText<HTMLInputElement>(en.cliCommand).value).toBe('codex')
    expect(screen.getByLabelText<HTMLInputElement>(en.cliTimeout).value).toBe('300000')
    // The unknown-namespace hint is gone: the family is curated.
    expect(screen.queryByText(new RegExp(en.advancedHint))).toBeNull()
    // No key field — neither CLI driver authenticates through a credentials reference.
    expect(screen.queryByLabelText(en.keyInput)).toBeNull()
    // The fold carries the model rows the runtime loads.
    fireEvent.click(screen.getByText(en.customized))
    expect(screen.getByLabelText<HTMLInputElement>(`${en.modelId} 1`).value).toBe('gpt-5-codex')
    expect(screen.getByText<HTMLButtonElement>(en.apply).disabled).toBe(false)
  })

  it('renders the dormant claude section with an empty timeout and no rows', async () => {
    const { face } = cliFace({ codexUser: undefined })
    const { ProviderEditor } = await import('../src/client/ProviderEditor.tsx')
    render(<ProviderEditor
      provider="claude-cli"
      displayName="Claude CLI"
      namespace={claudeCliNamespace(undefined)}
      schema={settingsSchema}
      settingsPath={['claude']}
      api={face}
      t={t}
      ...
      onClose={() => {}}
    />)
    const timeout = screen.getByLabelText<HTMLInputElement>(en.cliTimeout)
    expect(timeout.value).toBe('')
    expect(timeout.placeholder).toBe('')
    fireEvent.click(screen.getByText(en.customized))
    expect(screen.getByText(en.modelsEmpty)).toBeTruthy()
  })

  it('edits command and timeout as path ops under the driver section', async () => {
    const { mutate } = await mountCliCard()
    fireEvent.change(screen.getByLabelText(en.cliCommand), { target: { value: 'codex-nightly' } })
    fireEvent.change(screen.getByLabelText(en.cliTimeout), { target: { value: '240000' } })
    fireEvent.click(screen.getByText(en.apply))
    await waitFor(() => { expect(mutate).toHaveBeenCalledOnce() })
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ns: 'llm-claude-cli',
      ops: [
        { op: 'set', path: ['codex', 'command'], value: 'codex-nightly' },
        { op: 'set', path: ['codex', 'timeoutMs'], value: 240_000 },
      ],
      expectedRevision: 3,
    })
  })

  it('clearing the fields unsets the stored overrides', async () => {
    const { mutate } = await mountCliCard()
    fireEvent.change(screen.getByLabelText(en.cliCommand), { target: { value: '' } })
    fireEvent.change(screen.getByLabelText(en.cliTimeout), { target: { value: '' } })
    fireEvent.click(screen.getByText(en.apply))
    await waitFor(() => { expect(mutate).toHaveBeenCalledOnce() })
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ns: 'llm-claude-cli',
      ops: [
        { op: 'unset', path: ['codex', 'command'] },
        { op: 'unset', path: ['codex', 'timeoutMs'] },
      ],
      expectedRevision: 3,
    })
  })

  it('refuses an unparsable timeout before writing', async () => {
    const { mutate } = await mountCliCard()
    fireEvent.change(screen.getByLabelText(en.cliTimeout), { target: { value: 'soon' } })
    expect(screen.getByText(en.cliTimeoutInvalid)).toBeTruthy()
    expect(screen.getByText<HTMLButtonElement>(en.apply).disabled).toBe(true)
    expect(mutate).not.toHaveBeenCalled()
  })

  it('refuses a zero timeout before writing', async () => {
    const { mutate } = await mountCliCard()
    fireEvent.change(screen.getByLabelText(en.cliTimeout), { target: { value: '0' } })
    expect(screen.getByText(en.cliTimeoutInvalid)).toBeTruthy()
    expect(screen.getByText<HTMLButtonElement>(en.apply).disabled).toBe(true)
    expect(mutate).not.toHaveBeenCalled()
  })

  it('loads the runtime-loaded model list through llm.models and adopts new picks', async () => {
    const { mutate, models } = await mountCliCard()
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.fetchModels))
    // The picker offers what the live runtime loads; the configured model starts
    // unchecked, the runtime's new one checked.
    const dialog = await screen.findByRole('dialog', { name: en.fetchTitle })
    expect(models).toHaveBeenCalledWith({})
    expect(within(dialog).getByText('o4-mini')).toBeTruthy()
    ...
    expect(ids.map(input => input.value)).toEqual(['gpt-5-codex', 'o4-mini'])
    fireEvent.click(screen.getByText(en.apply))
    await waitFor(() => { expect(mutate).toHaveBeenCalledOnce() })
    expect(mutate.mock.calls[0]?.[0]).toMatchObject({
      ...
      ops: [{
        op: 'set',
        path: ['codex', 'models'],
        value: [{ id: 'gpt-5-codex', name: 'gpt-5-codex' }, { id: 'o4-mini', name: 'o4-mini' }],
      }],
    })
  })

  it('resets the model override back to inheritance and unsets the array', async () => {
    const { mutate } = await mountCliCard()
    fireEvent.click(screen.getByText(en.customized))
    expect(screen.getByText(en.modelsCustomized)).toBeTruthy()
    fireEvent.click(screen.getByText(en.resetModels))
    ...
    expect(mutate.mock.calls[0]?.[0]).toEqual({
      ns: 'llm-claude-cli',
      ops: [{ op: 'unset', path: ['codex', 'models'] }],
      expectedRevision: 3,
    })
  })

  it('reports an empty runtime catalog as no models to add', async () => {
    await mountCliCard({
      modelsGroups: [{ id: 'codex-cli', name: 'Codex CLI', models: [] }],
    })
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.fetchModels))
    await screen.findByText(en.fetchEmpty)
  ...
  it('names a dormant route when the runtime catalog has no group for it', async () => {
    await mountCliCard({ modelsGroups: [] })
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.fetchModels))
    await screen.findByText(en.runtimeRouteInactive)
  })

  it('surfaces a catalog failure from llm.models on the fetch action', async () => {
    await mountCliCard({ modelsFail: 'catalog down' })
    fireEvent.click(screen.getByText(en.customized))
    fireEvent.click(screen.getByText(en.fetchModels))
    await screen.findByText('catalog down')
  })
})
